...
import { PostRevision, FileAttachment, RemoveAttachment, PostCases, BindingList } from 'src/app/models/entities';
...
@Component({
  ...
  styleUrls: ['./case-post.component.scss']
})
export class CasePostComponent implements OnInit {
  public Data: PostCases = new PostCases();
  ...
  public statusList: BindingList[] = [];
  public needUpload: boolean = true;

  ngOnInit(): void {
  }

  public showDlg(currentStatus: string, status: BindingList[], needupload: boolean): void {
    this.Data.CaseID = this.api.case.Data.ID; 
    this.Data.Status = status[0].value;
  ...
  postCase(modal: any) {
    if (this.Data.Status.length == 0){
      this.isError = true;
      this.errorMessage = 'Next stage is unkwon';
      return;
    }
    this.isSaving = true;
    this.isError = false;
    this.api.casesPost(this.Data).subscribe(e => {
      this.isSaving = false;
      if (e.Success) {
        modal.close();
